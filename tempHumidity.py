...
while True:                                             #begin infinite loop
    ...
    tempF=10*float(splitPacket[0])                      #convert 1st array value to float      
    humidity=10*float(splitPacket[1])                   #convert 2nd array value to float
    #set temperature arrow
    theta=-2*np.pi/3069*tempF+5*np.pi/6                 #formula to get angle of line of potVal voltage
    tempArrow.axis=vector(arrowLength*np.cos(theta),
        arrowLength*np.sin(theta),0)
    #set humidity arrow
    theta=-2*np.pi/3069*humidity+5*np.pi/6 #formula to get angle of line of potVal voltage
    humidArrow.axis=vector(arrowLength*np.cos(theta),
        arrowLength*np.sin(theta),0)
    tempS=str(tempF/10)
    humS=str(humidity/10)
    lablT.text='Temperature = ' + tempS
    lablT.visible=True
    lablH.text='Humidity = ' + humS
    lablH.visible=True